...
type ResultRecord = {
  ...
  prize3: string;
  officialLink: string;
  redReferenceLink: string;
  blueReferenceLink: string;
  adminResult: string;
  updatedAt: number;
...
export default function Home() {
  ...
  const [followupFiles, setFollowupFiles] = useState<{ id: string; name: string; url: string }[]>([]);
  ...
  const readFollowupScreenshots = async (files: FileList | null) => {
    ...
    const preparedFiles = selected.map((file) => ({ id: crypto.randomUUID(), name: file.name, url: URL.createObjectURL(file) }));
    setFollowupFiles(preparedFiles);
  ...
  const deleteFollowupImage = (id: string) => {
    const target = followupFiles.find((file) => file.id === id);
    if (target) URL.revokeObjectURL(target.url);
    setFollowupFiles((current) => current.filter((file) => file.id !== id));
    setFollowupEntries((current) => current.filter((entry) => entry.sourceImageId !== id));
    setFollowupCopied(false);
  };
  const clearFollowupImages = () => {
    followupFiles.forEach((file) => URL.revokeObjectURL(file.url));
    setFollowupFiles([]);
    setFollowupEntries([]);
    setFollowupOcrText("");
    setFollowupError("");
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (nextDate !== activeDate) {
        ...
        setResultInput({ prize1: "", prize2: "", prize3: "", officialLink: "", redReferenceLink: "", blueReferenceLink: "", adminResult: "" });
      }
    }, 1_000);
    return () => window.clearInterval(timer);
  }, []);
  ...
  const saveResultShioSettings=async()=>{
    setResultError("");
    setResultShioNotice("");
    setResultShioSaving(true);
    try {
      const response=await fetch("/api/result-tracker",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"save-shio-settings",year:resultShioYear,settings:resultShioSettings.map(({name})=>({name}))})});
      const data=await response.json();
      if(!response.ok){setResultError(data.error||"Pengaturan shio gagal disimpan.");return;}
      await loadResultTracker();
      setResultShioNotice(`✓ Pengaturan shio ${resultShioYear} berhasil diperbarui.`);
      window.setTimeout(()=>setResultShioNotice(""),3500);
    } catch {
      setResultError("Pengaturan shio gagal disimpan. Periksa koneksi lalu coba lagi.");
    } finally {
      setResultShioSaving(false);
    }
  };
    ...
    <AuthGate>
      ...
      >
          ...
          >
            {kind === "followupBank" ? (
                ...
                <section className="followupBankColumns">
                  <section className="panel followupUploadPanel">
                    ...
                    <div className="followupBody">
                      ...
                      {!!followupFiles.length && <><div className="followupThumbs">{followupFiles.map((file) => <figure key={file.id}><button type="button" onClick={() => deleteFollowupImage(file.id)} title="Hapus gambar dan hasilnya">×</button><img src={file.url} alt={file.name} /><figcaption>{file.name}</figcaption></figure>)}</div><button type="button" className="followupClearAll" onClick={clearFollowupImages}>HAPUS SEMUA GAMBAR & HASIL</button></>}
                  ...
                  <section className="panel followupOutputPanel">
                    <div className="panelHead"><div><span>03</span><h2>Report LINE</h2></div><b>{followupEntries.length} TOTAL REKENING</b></div>
                    <div className="followupBody"><div className="followupTotal"><small>TOTAL HASIL FOLLOW-UP</small><strong>{followupEntries.length}</strong><span>REKENING BERMASALAH / OFF</span></div><pre className="followupReport">{followupEntries.length ? followupReportText : "Hasil report akan tampil setelah screenshot selesai dibaca."}</pre><button className="bankOffCopy" disabled={!followupEntries.length} onClick={copyFollowupReport}>{followupCopied ? "✓ REPORT TERSALIN" : "SALIN REPORT UNTUK LINE"}</button></div>
                  </section>
                </section>
              </section>
            ) : kind === "bankOff" ? (
              <section className="bankOffStudio">
                ...
                <div className="bankOffMaintenanceNote">
                  <strong>NOTE:</strong>
                  <span>JIKA TERDAPAT REKENING YANG SUDAH DICABUT HARAP DIHAPUS PADA DATA BANK SITUS, DAN JIKA ADA PENAMBAHAN BANK SILAKAN DITAMBAHKAN PADA DATA BANK SITUS YA KO/CI.</span>
                </div>
              ...
              <section className="resultTrackerStudio">
                ...
                {(["pagi", "malam"] as const).map((shift) => (
                  <section className={`resultShiftSection shift-${shift}`} key={shift}>
                ...
                {resultEditor && <div className="resultModalBackdrop"><section className="resultModal"><button className="resultModalClose" onClick={() => setResultEditor(null)}>×</button><small>INPUT RESULT HARI INI</small><h2>{resultEditor.name}</h2><p>{getWibIsoDate()} · Result {resultEditor.resultTime} WIB</p><div className="resultPrizeInputs"><label><span>Prize 1</span><input value={resultInput.prize1} onChange={(event) => setResultInput((old) => ({ ...old, prize1: event.target.value }))} /></label><label><span>Prize 2</span><input value={resultInput.prize2} onChange={(event) => setResultInput((old) => ({ ...old, prize2: event.target.value }))} /></label><label><span>Prize 3</span><input value={resultInput.prize3} onChange={(event) => setResultInput((old) => ({ ...old, prize3: event.target.value }))} /></label></div><div className="resultReferenceInputs"><label><span>Link Resmi</span><input value={resultInput.officialLink} onChange={(event) => setResultInput((old) => ({ ...old, officialLink: event.target.value }))} placeholder="https://link-resmi..." /></label><label className="red"><span>Acuan Merah</span><input value={resultInput.redReferenceLink} onChange={(event) => setResultInput((old) => ({ ...old, redReferenceLink: event.target.value }))} placeholder="https://acuan-merah..." /></label><label className="blue"><span>Acuan Biru</span><input value={resultInput.blueReferenceLink} onChange={(event) => setResultInput((old) => ({ ...old, blueReferenceLink: event.target.value }))} placeholder="https://acuan-biru..." /></label></div><label><span>Link / Hasil Result Admin</span><textarea rows={4} value={resultInput.adminResult} onChange={(event) => setResultInput((old) => ({ ...old, adminResult: event.target.value }))} placeholder="Tempel link screenshot hasil admin (https://...) atau keterangan admin..." /></label><button className="primary" onClick={saveResultRecord}>SIMPAN HASIL RESULT</button></section></div>}
              </section>
            ) : kind === "resultArchive" ? (
              <section className="resultArchiveStudio">
                <section className="panel resultTrackerHero resultArchiveHero">
                  <div>
                    ...
                    <h1>Arsip Result &amp; Shio</h1>
                    <p>Cari hasil pasaran berdasarkan tanggal dan kelola acuan shio tahunan.</p>
                  </div>
                  <div className="resultLiveClock">
                    <small>WAKTU INDONESIA BARAT</small>
                    <b>{new Intl.DateTimeFormat("id-ID", { timeZone: "Asia/Jakarta", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(resultClock)} WIB</b>
                ...
                <section className="panel resultArchivePanel">
                  ...
                  <div className="resultArchiveTable"><table><thead><tr><th>TANGGAL</th><th>SHIFT</th><th>PASARAN</th><th>PRIZE 1</th><th>PRIZE 2</th><th>PRIZE 3</th><th>LINK RESMI</th><th>ACUAN MERAH</th><th>ACUAN BIRU</th><th>HASIL ADMIN</th><th>UPDATE</th></tr></thead><tbody>{resultArchive.length ? resultArchive.map((record) => <tr key={record.id}><td>{record.resultDate}</td><td>{record.shift.toUpperCase()}</td><td>{record.marketName}</td><td>{record.prize1 || "-"}</td><td>{record.prize2 || "-"}</td><td>{record.prize3 || "-"}</td><td>{record.officialLink ? <a href={record.officialLink} target="_blank" rel="noreferrer">BUKA</a> : "-"}</td><td>{record.redReferenceLink ? <a href={record.redReferenceLink} target="_blank" rel="noreferrer">BUKA MERAH</a> : "-"}</td><td>{record.blueReferenceLink ? <a href={record.blueReferenceLink} target="_blank" rel="noreferrer">BUKA BIRU</a> : "-"}</td><td>{record.adminResult ? (/^https?:\/\//i.test(record.adminResult.trim()) ? <a href={record.adminResult.trim()} target="_blank" rel="noreferrer">LIHAT HASIL ADMIN</a> : record.adminResult) : "-"}</td><td>{record.updatedByName || "-"}</td></tr>) : <tr><td colSpan={11}>Belum ada arsip pada filter ini.</td></tr>}</tbody></table></div>
                </section>
                <section className="panel resultShioPanel">
                  <div className="panelHead"><div><span>02</span><h2>Pengaturan Shio Result Tahunan</h2></div><span className="live">● ANGKA DIKUNCI SISTEM</span></div>
                  ...
                  <div className="resultShioLockInfo">🔒 Nomor acuan dikunci permanen. Master hanya dapat mengubah nama atau urutan shio.</div>
                  <div className="resultShioGrid">{resultShioSettings.map((item,index)=><label key={`${index}`}><input className="shioName" value={item.name} disabled={!canManageUsers} onChange={(event)=>setResultShioSettings(old=>old.map((entry,i)=>i===index?{...entry,name:event.target.value.toUpperCase()}:entry))}/><input className="shioNumbersLocked" value={item.numbers} readOnly tabIndex={-1} title="Nomor acuan dikunci oleh sistem" aria-label={`Nomor acuan ${item.name} terkunci`}/></label>)}</div>
                  {canManageUsers&&<div className="resultShioActions"><button className="resultShioSave" disabled={resultShioSaving} onClick={saveResultShioSettings}>{resultShioSaving?"MENYIMPAN PERUBAHAN...":`UPDATE & SIMPAN SHIO ${resultShioYear}`}</button>{resultShioNotice&&<div className="resultShioNotice">{resultShioNotice}</div>}</div>}
                </section>
              </section>
            ) : kind === "handover" ? (
        ...
        <style>{`
          ...
          .resultReadyCopy{display:grid;gap:8px;padding:12px;border:1px solid rgba(87,242,192,.3);border-radius:10px;background:#020b0e}.resultReadyCopy>small{color:#57f2c0;font-size:8px;font-weight:900;letter-spacing:.08em}.resultReadyCopy pre{max-height:145px;margin:0;overflow:auto;color:#eaffff;font:700 9px/1.55 "Lexend",Arial,sans-serif;white-space:pre-wrap}.resultReadyCopy button{min-height:38px;border:1px solid #42eab4;border-radius:7px;color:#001811;background:#57f2c0;font:900 9px "Lexend",Arial,sans-serif}.resultShioPanel{overflow:hidden}.resultShioToolbar{display:flex;align-items:end;gap:18px;padding:18px 20px}.resultShioToolbar label{display:grid;gap:7px}.resultShioToolbar span{color:#80a1a8;font-size:8px;font-weight:900}.resultShioToolbar input{width:150px;min-height:42px;padding:0 11px;border:1px solid rgba(0,207,255,.25);border-radius:8px;color:#fff;background:#020b0f;font:800 11px "Lexend",Arial,sans-serif}.resultShioToolbar p{margin:0 0 10px;color:#8faab0;font-size:9px}.resultShioLockInfo{margin:0 20px 14px;padding:12px 14px;border:1px solid rgba(255,211,74,.3);border-radius:9px;color:#f4d76d;background:rgba(255,205,43,.07);font:800 9px/1.5 "Lexend",Arial,sans-serif}.resultShioGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:0 20px 20px}.resultShioGrid label{display:grid;grid-template-columns:130px 1fr;gap:8px}.resultShioGrid input{min-width:0;min-height:42px;padding:0 11px;border:1px solid rgba(0,207,255,.2);border-radius:8px;color:#dffaff;background:#031116;font:700 9px "Lexend",Arial,sans-serif}.resultShioGrid .shioName{color:#ffe600;font-weight:900}.resultShioGrid input:disabled{opacity:.82}.resultShioGrid .shioNumbersLocked{border-style:dashed;color:#9eb2b7;background:repeating-linear-gradient(135deg,#07171c,#07171c 8px,#091c22 8px,#091c22 16px);cursor:not-allowed;user-select:none}.resultShioGrid .shioNumbersLocked:focus{outline:none;box-shadow:none}.resultShioActions{display:flex;align-items:center;gap:14px;padding:0 20px 22px}.resultShioPanel .resultShioSave{min-height:50px;padding:0 22px;border:1px solid #ffe267;border-radius:10px;color:#041211!important;background:linear-gradient(105deg,#ffe36b 0%,#5ceeb0 52%,#52d9f2 100%)!important;box-shadow:0 9px 28px rgba(74,229,192,.2),inset 0 1px rgba(255,255,255,.65)!important;filter:none!important;opacity:1!important;font:900 10px "Lexend",Arial,sans-serif;letter-spacing:.04em;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease}.resultShioPanel .resultShioSave:hover{transform:translateY(-2px);box-shadow:0 12px 34px rgba(74,229,192,.32)!important}.resultShioPanel .resultShioSave:disabled{transform:none;cursor:wait;opacity:.68!important}.resultShioNotice{padding:11px 14px;border:1px solid rgba(73,243,183,.4);border-radius:9px;color:#65f3bd;background:#073127;font:800 9px "Lexend",Arial,sans-serif;animation:noticeIn .2s ease}@keyframes noticeIn{from{opacity:0;transform:translateY(4px)}}.shell.lightMode .resultReadyCopy,.shell.lightMode .resultShioGrid input,.shell.lightMode .resultShioToolbar input{color:#17383f!important;background:#fff!important}.shell.lightMode .resultReadyCopy pre{color:#17383f!important}.shell.lightMode .resultShioGrid .shioNumbersLocked{color:#66787c!important;background:repeating-linear-gradient(135deg,#eef4f5,#eef4f5 8px,#e4edef 8px,#e4edef 16px)!important}.shell.lightMode .resultShioLockInfo{color:#725500;background:#fff8d9}.shell.lightMode .resultShioNotice{color:#086e4e;background:#ddfff2}
          ...
          .resultReferenceLinks{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));width:100%}.resultReferenceLinks a{min-width:0;text-align:center}.resultReferenceLinks a.official{border-color:#69d9bb;color:#72efc9;background:#07251d}.resultReferenceLinks a.red{border-color:#ff6075;color:#ff9aa8;background:#351019}.resultReferenceLinks a.blue{border-color:#45a8ff;color:#8bcbff;background:#081d38}.resultReferenceInputs{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.resultReferenceInputs label.red span{color:#ff8495}.resultReferenceInputs label.red input{border-color:rgba(255,84,108,.5)}.resultReferenceInputs label.blue span{color:#72bdff}.resultReferenceInputs label.blue input{border-color:rgba(58,158,255,.55)}@media(max-width:760px){.resultReferenceInputs,.resultReferenceLinks{grid-template-columns:1fr}}
          ...
          /* Pemisah shift dibuat tegas tanpa mengubah data pasaran. */
          .resultShiftSection{gap:10px;min-height:0}.resultShiftSection+.resultShiftSection{margin-top:4px}.resultMarketCards{align-items:start}
          .resultShiftHead{position:sticky;top:10px;z-index:18;min-height:58px;padding:11px 16px;border:1px solid;border-radius:12px;backdrop-filter:blur(14px);box-shadow:0 10px 30px rgba(0,0,0,.24)}
          .resultShiftHead>div{gap:12px}.resultShiftHead span{display:grid;place-items:center;width:34px;height:34px;border-radius:10px;font-size:20px}.resultShiftHead h2{font-size:23px;font-weight:950;letter-spacing:.055em;text-shadow:0 0 16px currentColor}.resultShiftHead>b{font-size:9px;font-weight:950;letter-spacing:.06em}
          .shift-pagi .resultShiftHead{border-color:rgba(255,210,69,.5);color:#ffe06b;background:linear-gradient(90deg,rgba(70,49,4,.94),rgba(9,27,31,.96) 72%)}
          .shift-pagi .resultShiftHead span{color:#171000;background:#ffd84f;box-shadow:0 0 18px rgba(255,216,79,.32)}
          .shift-pagi .resultShiftHead>b{border-color:rgba(255,216,79,.4);color:#ffe995;background:rgba(80,57,4,.55)}
          .shift-malam{padding-top:2px;border-top:3px solid transparent;border-image:linear-gradient(90deg,#6a76ff,#b66cff,#25d8ff) 1}
          .shift-malam .resultShiftHead{border-color:rgba(130,127,255,.58);color:#d7d8ff;background:linear-gradient(90deg,rgba(29,25,91,.97),rgba(7,20,39,.97) 68%,rgba(15,52,72,.96));box-shadow:0 10px 34px rgba(70,79,255,.18)}
          .shift-malam .resultShiftHead span{color:#fff;background:linear-gradient(145deg,#7774ff,#402d9f);box-shadow:0 0 19px rgba(111,104,255,.48)}
          .shift-malam .resultShiftHead h2{color:#f0efff;text-shadow:0 0 18px rgba(131,125,255,.8)}
          .shift-malam .resultShiftHead>b{border-color:rgba(118,149,255,.48);color:#dce8ff;background:rgba(37,48,105,.68)}
          .shift-malam .resultMarketCard{border-color:rgba(110,121,255,.52);background:linear-gradient(145deg,#0b1830,#030a13)}.shift-malam .resultMarketCard::before{background:#7976ff;box-shadow:0 0 14px rgba(121,118,255,.65)}.shift-malam .resultMarketCardTop small{color:#a9a7ff}
          .shell.lightMode .resultShiftHead{backdrop-filter:none}.shell.lightMode .shift-pagi .resultShiftHead{color:#604500;background:linear-gradient(90deg,#fff2b9,#f8fcfd 72%)}.shell.lightMode .shift-malam .resultShiftHead{color:#24205f;background:linear-gradient(90deg,#e3e3ff,#eef8ff)}.shell.lightMode .shift-malam .resultShiftHead h2{color:#29246c;text-shadow:none}
          ...
          .followupThumbs figure{position:relative}.followupThumbs figure>button{position:absolute;top:2px;right:2px;z-index:3;display:grid;place-items:center;width:21px;height:21px;padding:0;border:1px solid #ff7c8d;border-radius:50%;color:#fff;background:#9e2033;font-size:13px;font-weight:950;box-shadow:0 4px 12px rgba(0,0,0,.35)}.followupClearAll{min-height:38px;border:1px solid #803443;border-radius:8px;color:#ffacb8;background:#32131a;font:900 8px "Lexend",sans-serif}.followupTotal{display:grid;grid-template-columns:1fr auto;align-items:center;gap:4px 12px;padding:13px 15px;border:1px solid rgba(255,194,84,.36);border-radius:10px;background:linear-gradient(135deg,#251a08,#0b2022)}.followupTotal small{color:#a7bbbf;font-size:7px;font-weight:900}.followupTotal strong{grid-row:1/3;grid-column:2;color:#ffe173;font-size:28px}.followupTotal span{color:#f2cb72;font-size:8px;font-weight:900}.shell.lightMode .followupTotal{background:linear-gradient(135deg,#fff4d4,#eaf5f4)}.shell.lightMode .followupClearAll{color:#9c2435;background:#fff0f2}
